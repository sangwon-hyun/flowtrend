...
(Update Nov 2023: I think I'm retiring this. The ADMM objective value isn't guaranteed to drop, and a plateau might be a wrong criterion.)
...
```{r}
usethis::use_c("C_prox_dp.c")
```

Then, the C code is here:

```{c C_prox_dp, send_to = "src/C_prox_dp.c"}
#include <R.h>
...
void C_prox_dp(int *pn, double *y, double *plam, double *beta) {
...
Jacob says: the `send_to` argument tells litr to send the code chunk to the file
`src/C_prox_dp.c`.  If the file does not exist, it will be created.  If it does
exist, the code chunk will be appended to the end of the file.
