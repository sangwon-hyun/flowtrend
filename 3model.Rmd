...
Here's a simple test, against an alternative function that only works for
equally spaced data.
...
```{r test_gen_tf_mat_equalspace, eval = FALSE}
...
TODO: think about best way to design sigma
...
Next, we define the main function `Mstep_mu()`.  This uses a "locally-adaptive"
ADMM [TODO: add source]. `M-step_mu()` calls `la_admm_oneclust()` on each
cluster $k=1,\cdots, K$; this function will be introduced shortly.
...
[TODO: I might rewrite this into Cpp]

```{C prox_dp, eval = FALSE}
#include <R.h>
...
void prox_dp(int *pn, double *y, double *plam, double *beta) {
...
First, let's start with a few R functions.
...
Here's a function to compute the augmented Lagrangian.
[TODO: write the augmented Lagrangian here]
...
Here's a function to calculate the ADMM objective.

[TODO: write the admm objective here]
...
## Ported over from flowmix

There are some potentially useful computational tricks from flowmix (e.g. the
...
#' We might not use this. Legacy from flowmix.
...
- Right now, `sigma_eig_by_clust` is not used. When speeding up the code, do
  this first.
- Likewise, `ycentered_list` isn't used, while it is clearly useful in Sigma M
