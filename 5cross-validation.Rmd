...
```{r predict_flowsmooth, eval = TRUE}
...
```{r test-predict_flowsmooth, eval = TRUE}
...
```{r test-predict_flowsmooth2, eval = TRUE}
## Generate data
set.seed(100)
dt       <- gendat_1d(100, rep(100, 100), die_off_time = 0.45)
...
```{r predict-plot-mean, eval = TRUE, fig.width = 7, fig.height = 5}
predobj = predict_flowsmooth(obj, newtimes = held_out)
...
```{r predict-plot-prob, eval = TRUE, fig.width = 7, fig.height = 5}
plot_prob(obj, x=x) +
...
The estimated $\mu$ in the training data are shown as solid triangle points. The
out-of-sample $\mu$ predictions made on a fine grid of time points (shown by the
yellow lines) look fine.


```{r uneven-mean-plot, eval = TRUE, fig.width = 7, fig.height = 5}
g = plot_1d(ylist, obj, x = x)
...
```{r uneven-prob-plot, eval = TRUE, fig.width = 7, fig.height = 5}
plot_prob(obj, x=x) +
  geom_line(aes(x = time, y = prob, group = cluster, color = cluster),
            data = dt_model, linetype = "dashed")  +
  facet_wrap(~cluster) 
...
The fit is similar for the predicted model and the estimated model.

```{r before-push-away, fig.width = 7, fig.height = 5})
...
```{r push-away, fig.width = 7, fig.height = 5}
...
Okay, now we know it's *possible* to run on a laptop.
...
- ADMM convergence <-- detailed look
- ADMM sub-steps
- density calculation
- spaced CV correctness check.
