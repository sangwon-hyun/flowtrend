...
```{r temp-load-flowtf}
devtools::load_all("~/repos/FlowTF") ## Temporary! flowtrend hasn't fully ported over the m-step yet.
```
...
for(refit in c(FALSE, TRUE)){
  cv_flowtrend(ylist = ylist, countslist = NULL, 
                ...
                lambda_probs = lambda_probs,
                l = l, l_prob = l_prob,
                maxdev = NULL, numclust = 3, nfold = 5, nrestart = 5,
                verbose = TRUE, refit = refit, save_meta = TRUE, mc.cores = 6)
}

## Test out the summary
obj = cv_summary(destin = "~/repos/flowtrend/tempoutput")
...
get_max_lambda("~/repos/flowtrend/tempoutput",
...
- ADMM convergence <-- take a detailed look
